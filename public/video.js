...
(function () {
  ...
  function setupAnnotationTools() {
    ...
    textBtn.addEventListener('click', () => startTextAnnotation());
    drawBtn.addEventListener('click', () => startDrawAnnotation());
  }

  function startTextAnnotation() {
    ...
    instructions.className = 'annot-instructions';
    instructions.textContent = 'Click where you want to place text';
    overlay.appendChild(instructions);

    const cancelBtn = document.createElement('button');
    cancelBtn.className = 'annot-cancel-btn';
    cancelBtn.textContent = 'Cancel';
    cancelBtn.addEventListener('click', removeAnnotationOverlay);
    ...
    overlay.addEventListener('click', function onOverlayClick(e) {
      ...
      // Replace instructions with text input at click position
      instructions.remove();
      ...
      textInput.style.cssText = `position:absolute; left:${Math.min(x, rect.width - 200)}px; top:${Math.max(y - 16, 4)}px;`;
      overlay.appendChild(textInput);

      const doneBtn = document.createElement('button');
      doneBtn.className = 'annot-done-btn';
      ...
      async function captureAndPost() {
        const labelText = textInput.value.trim() || '[Text annotation]';
        const commentLabel = textInput.value.trim() || '[Text annotation]';

        // Capture video frame + annotation text to canvas
        ...
        // Scale x/y to canvas dimensions
        ...
        canvas.toBlob(async (blob) => {
          await postAnnotationComment(commentLabel, blob);
        }, 'image/png');
      }

      doneBtn.addEventListener('click', captureAndPost);
      textInput.addEventListener('keydown', e => {
        if (e.key === 'Enter') { e.preventDefault(); captureAndPost(); }
        if (e.key === 'Escape') removeAnnotationOverlay();
      });
    });
  }

  function startDrawAnnotation() {
    if (videoEl.paused === false) videoEl.pause();
    const wrapper = document.querySelector('.video-wrapper');
    ...
    overlay.id = 'annot-overlay';
    overlay.className = 'annot-overlay';
    overlay.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;z-index:50;';

    const drawCanvas = document.createElement('canvas');
    drawCanvas.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;cursor:crosshair;touch-action:none;';
    ...
    // Size draw canvas to match display size
    const rect = overlay.getBoundingClientRect();
    ...
    doneBtn.addEventListener('click', async () => {
      // Capture video frame + drawing overlay
      const outCanvas = document.createElement('canvas');
      outCanvas.width = videoEl.videoWidth || videoEl.clientWidth;
      outCanvas.height = videoEl.videoHeight || videoEl.clientHeight;
      const outCtx = outCanvas.getContext('2d');
      outCtx.drawImage(videoEl, 0, 0, outCanvas.width, outCanvas.height);

      // Scale drawing canvas to output size
      outCtx.drawImage(drawCanvas, 0, 0, outCanvas.width, outCanvas.height);

      removeAnnotationOverlay();

      outCanvas.toBlob(async (blob) => {
        await postAnnotationComment('[Drawing]', blob);
      }, 'image/png');
    });
  }

  function removeAnnotationOverlay() {
    const existing = document.getElementById('annot-overlay');
    if (existing) existing.remove();
  }

  async function postAnnotationComment(text, imageBlob) {
    const displayName = localStorage.getItem('feedo_display_name') || 'Admin';
    ...
    formData.append('attachment', imageBlob, 'annotation.png');
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        showToast(err.error || 'Failed to post annotation', 'error');
        return;
      }
